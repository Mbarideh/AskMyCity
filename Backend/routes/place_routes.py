...
def explore_places(
    ...
    """Browse the public directory without invoking AI search.

    Business Studio records are included only when they are published/active.
    This endpoint intentionally stays database-only so Explore is fast and does
    not create enrichment/API cost.
    """
    statement = select(Place).where(
        or_(
            Place.source != "business_studio",
    ...
    if search and search.strip():
        ...
        statement = statement.where(
            or_(
                ...
                Place.subtypes.ilike(search_value),
                Place.description.ilike(search_value),
            )
        )

    if city and city.strip():
        city_value = f"%{city.strip()}%"
        statement = statement.where(Place.city.ilike(city_value))
